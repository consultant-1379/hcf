#ifndef __INCLUDE_GUARD_CLASS_OM_IMM_HANDLER_IMPL__
#define __INCLUDE_GUARD_CLASS_OM_IMM_HANDLER_IMPL__ om_handler_impl

/**
 *  @file om_handler_impl.h
 *	@brief
...
public:
	 ...
	 */
	inline unsigned long long int getCcbHandleValue () { return _ccbHandle; }
...
private:
	 ...
	 * and to set in _exitCodeStringRetrieved variable the found string
	 *
...
private:
	...
	//========//
private:
	SaImmHandleT _immHandle;
	SaImmCcbHandleT _ccbHandle;
	SaVersionT _version;
	SaImmAdminOwnerHandleT _ownerHandle;
	SaImmAccessorHandleT _accessorHandle;
	int _errorCode;
	std::map<std::string, SaImmCcbHandleT> _transactionList;
	std::map<std::string, SaImmAdminOwnerHandleT> _transactionListOwnerHandle;
	SaImmCcbFlagsT _ccbFlagVal;
	char _processName[1024];
	std::string _exitCodeStringRetrieved;
	SaNameT _objOwnedName;
	const SaNameT * _objOwnedNameList[2];
};

TSL_NS_HCF_IMM_END

#endif // __INCLUDE_GUARD_CLASS_OM_IMM_HANDLER_IMPL__
